...
UPDATE sales s
INNER JOIN products pON s.product_name = p.product_name
...
ORDER BY c.customer_name, p.category; # 여기는 모든 경우의 수를 곱하기 때문에 ON 필요 없음
...
FROM  customers c
CROSS JOIN products p
WHERE c.customer_type = 'VIP'	# VIP고객이면서
	AND NOT EXISTS(				# 구매하지 않은 상품
		SELECT 1 FROM sales s	# 서브쿼리의 리턴값(1, NULL)
